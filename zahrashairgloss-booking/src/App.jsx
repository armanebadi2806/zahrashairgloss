...
const supabaseApi = async (path, options={}) => {
  ...
  if(path.startsWith('/api/admin/bookings/')&&path.endsWith('/payment')&&method==='POST'){await rpc('admin_mark_booking_paid',{p_id:path.split('/')[4]});return {paid:true};}
...
function Admin({ onExit, onLoggedOut }) {
  ...
  const confirmDeposit=async()=>{try{await api(`/api/admin/bookings/${selectedBooking.id}/payment`,{method:'POST',body:'{}'});setSelectedBooking((current)=>current?{...current,paymentStatus:'paid'}:current);flash('Anzahlung bestätigt.');await refresh();}catch(err){setError(err.message);}};
